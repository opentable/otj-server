...
public class PredicateForwardedHeaderCustomizer implements ForwardedHeaderCustomizer {
    ...
    public static ForwardedHeaderCustomizer regexURI(Pattern pattern) {
        return new PredicateForwardedHeaderCustomizer(serverHttpRequest -> pattern.matcher(serverHttpRequest.getPath()
                .pathWithinApplication().toString()).matches());
    }

    Predicate<ServerHttpRequest> predicate;

    public PredicateForwardedHeaderCustomizer(Predicate<ServerHttpRequest> predicate) {
        this.predicate = predicate;
    }

    @Override
    public boolean shouldFilter(ServerHttpRequest httpServletRequest) {
